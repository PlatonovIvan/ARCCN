...
class Discovery (EventMixin):
  ...
  def __init__ (self, install_flow = True, explicit_drop = True):
    ...
    self.addListener(SwitchReadyEvent, SwitchReadyEvent)
  ...
  def _handle_PacketIn (self, event):
    ...
    if link not in self.adjacency:
      ...
      if (not self.packet_number.has_key(dpid)):
        self.packet_number[dpid]=1
      else:
        self.packet_number[dpid]+=1
      if (self.packet_number[dpid]==LINK_NUMBER):
        self.raiseEvent(SwitchReadyEvent(dpid))
