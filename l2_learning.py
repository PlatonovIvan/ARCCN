...
class LearningSwitch (EventMixin):
  ...
  def RespondToARP(self, packet, match, event):
    ...
    if (match.nw_dst == addresses.IPAddr("10.0.1.1")):
      r.protosrc = addresses.IPAddr("10.0.1.1")
      r.hwsrc = addresses.EthAddr("ca:72:aa:6e:b3:83")
    if (match.nw_dst == addresses.IPAddr("10.0.2.1")):
      r.protosrc = addresses.IPAddr("10.0.2.1")
      r.hwsrc = addresses.EthAddr("06:57:b2:ee:76:88")
  ...
  def _handle_PacketIn (self, event):
    ...
    if ( match.dl_type == packet.ARP_TYPE and
      ...
      match.nw_dst == addresses.IPAddr("10.0.1.1") or
      match.nw_dst == addresses.IPAddr("10.0.2.1"))):
